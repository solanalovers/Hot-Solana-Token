...
export default function HomeFilter({ filter, handleChangeFilter }: any) {
  ...
  return (
    <Flex marginTop={'32px'} marginBottom={'24px'} columnGap={'60px'} alignItems={'center'}>
      <Text
        color={text}
        fontSize={'14px'}
        lineHeight={'22px'}
      ...
      <SearchBox />
      <FilterSelect filter={filter.top} filterList={topFilter} handleChangeFilter={(value: string) => handleChangeFilter(value, 'top')} />
    </Flex>
  )
}
